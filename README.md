...
В текущей реализации вероятность "удержать" предмет после успешного захвата берется не из `machines[].economy.dropAfterGrabChance`, а из поля `rewards[].chance`.

Это значит:

- чем выше `rewards[].chance`, тем выше вероятность сохранить уже выбранную награду;
- `chance = 0` означает, что предмет всегда выпадет из клешни на этом этапе;
- `chance = 1` означает, что предмет никогда не выпадет из клешни на этом этапе.
...
| `economy.dropAfterGrabChance`        | `number` | Поле есть в конфиге, но в текущем коде не участвует в финальном resolve. Фактическая вероятность удержать предмет сейчас берется из `rewards[].chance`. |
...
| `chance`   | `number` от `0` до `1` | Вероятность удержать уже выбранную награду после успешного захвата. `1` означает удержать всегда, `0` означает уронить всегда. |
...
- Если random roll больше `chance`, сервер меняет результат с `win` на `lose`.

Следствие:

- маленькое значение `chance` делает награду менее "выдаваемой";
- большое значение `chance` делает награду более достижимой;
- `chance = 1` отключает этот этап потери предмета;
- `chance = 0` фактически гарантирует срыв после выбора награды.
